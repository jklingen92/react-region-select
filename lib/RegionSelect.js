	...
	_createClass(RegionSelect, [{
		...
		value: function onDocMouseTouchEnd() {
			if (this.isChanging) {
				...
				if (updatingRegion.width >= this.props.minWidth && updatingRegion.height >= this.props.minHeight && updatingRegion.width <= this.props.maxWidth && updatingRegion.height <= this.props.maxHeight) {
					...
					this.props.onChange([].concat(_toConsumableArray(this.props.regions.slice(0, index)), _toConsumableArray(this.props.regions.slice(index + 1))));
				}
			}
		}
	}, {
		...
		value: function onComponentMouseTouchDown(event) {
			...
			var rect = {
				...
				height: 0,
				new: true,
				data: _extends({}, this.props.defaultData),
				isChanging: true
			};
		...
		value: function renderRect(rect, index) {
			...
			var Region = this.props.regionElement;
			var regionProps = this.props.regionProps;
			return _react2.default.createElement(Region, _extends({
				x: rect.x,
				y: rect.y,
				width: rect.width,
				height: rect.height,
...
RegionSelect.propTypes = {
	...
	regionElement: _propTypes.PropTypes.element,
	regionProps: _propTypes.PropTypes.object,
	...
	defaultData: _propTypes.PropTypes.object,
	minWidth: _propTypes.PropTypes.number,
	minHeight: _propTypes.PropTypes.number,
	maxWidth: _propTypes.PropTypes.number,
...
RegionSelect.defaultProps = {
	...
	constraint: false,
	disabled: false,
	regionElement: _Region2.default,
	regionProps: null,
	defaultData: null,
	minWidth: 0.1,
	minHeight: 0.1,
